...
func teaHandler(database *db.DB) func(s ssh.Session) (tea.Model, []tea.ProgramOption) {
	return func(s ssh.Session) (tea.Model, []tea.ProgramOption) {
		...
		if ok && isAdmin {
			...
			model, err := tui.NewModel(s.Context(), database, "fingerprint", false, true, nil, conn)
			if err != nil {
		...
		model, err := tui.NewModel(s.Context(), database, fingerprint, anonymous, false, &host, conn)
